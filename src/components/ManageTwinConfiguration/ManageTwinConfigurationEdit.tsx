...
const ManageTwinConfigurationEdit: React.FC<ManageTwinConfigurationProps> = ({
  isLoading,
  config,
  integrationSources,
  closeModal
  ...
  const { mutate: createConfiguration, isPending: isCreatePending } = useMutation({
    mutationKey: ['configuration-create', twinId],
    mutationFn: async (variables: { twinId: string, payload: TwinConfigurationCreateRequestPayload }) => {
      const { twinId, payload } = variables
      return await twinConfigurationService.createConfiguration(twinId, payload)
    },
    onSuccess: (data: AxiosResponse<TwinConfigurationCreateResponse>) => {
  ...
  const { mutate: editConfiguration, isPending: isEditPending } = useMutation({
    mutationKey: ['configuration-edit', twinId],
    mutationFn: async (variables: { twinId: string, payload: TwinConfigurationCreateRequestPayload }) => {
      const { twinId, payload } = variables
      return await twinConfigurationService.editConfiguration(twinId, payload)
    },
    onSuccess: (data: AxiosResponse<TwinConfigurationCreateResponse>) => {
      dispatch(alertActions.triggerAlert({
    ...
    if (selectedType?.source === CONFIGURATION_STORAGE_TYPES.INTERNAL) {
      internalInputValuesSchema.validate(inputValues).then(res => {
        ...
        if (isEditMode) {
          ...
          editConfiguration({ twinId, payload })
        } else {
          createConfiguration({ twinId, payload })
        }
      }).catch(() => {
        dispatch(alertActions.triggerAlert({
...
export default ManageTwinConfigurationEdit
